...
if ( ! class_exists('Nivo_Image_Slider_Meta_Box') ):

class Nivo_Image_Slider_Meta_Box
{
	public function __construct()
	{
		add_action( 'save_post', array( $this, 'save_meta_boxes' ) );
		add_action( 'wp_ajax_nivo_image_slider_save_images', array( $this, 'save_images' ) );
	}
	...
	{
	    if ( ! isset($_POST['nonce']) || ! wp_verify_nonce( $_POST['nonce'], 'nivo_image_slider_ajax_nonce' ) ) {
	        return;
	    }
	...
	private function file($field, $name, $value)
	{
        ...
        <input type="button" class="button" id="<?php echo $btn_id; ?>" value="<?php echo $btn_browse; ?>"><br class="clear" />
        <textarea name="<?php echo $name;?>" class="regular-text" id="<?php echo $field['id'];?>" cols="30" rows="5" style="width: 100%;"><?php echo $value;?></textarea>
		<script>
			jQuery(function($){
				var frame,
						...
						insert: {
							...
							click: function() {
								var models = frame.state().get('selection'),
									attachment_id = models.first().attributes.url,
									files = [];
	...
	public function images( $field, $name, $value )
	{
		...
		$html  = '';
		$html .= '<div class="nivo_image_slider_images">';
		$html .= sprintf('<input type="hidden" value="%1$s" id="nivo_image_slider_images_ids" name="_slider_images">', $value);
		$html .= sprintf('<span id="nivo_image_slider_btn" class="button button-default">%s</span>', $btn_text);
		$html .= sprintf('<br class="clear"><ul class="nivo_image_slider_list">%s</ul>', $output);
		$html .= '</div>';
...
if(is_admin()){
	$Nivo_Image_Slider_Meta_Box = new Nivo_Image_Slider_Meta_Box();
}
